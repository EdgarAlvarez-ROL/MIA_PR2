    ...
    def fdisk(self):
        ...
        if not eliminar:
            ...
            if size <= 0 or path == '' or name == '':
                ...
                self.salida_consola("\tERROR: Faltan parámetros obligatorios para el comando FDISK\n")
                return
            if not add:
            ...
            if required:
                ...
                self.salida_consola("\tERROR: Faltan parámetros obligatorios para el comando FDISK junto con el parámetro DELETE\n")
                return
        ...
        try:
            ...
            if u.lower() in ["b", "k", "m"]:
                if u.lower() != "b":
                    if u.lower() == "k":
                        ...
                        # print(f"La unit {u} es M")
                        i *= 1024 * 1024
                        # print(f"la i que es size vale ")
            else:
            ...
            for prttn in partitions:
                ...
                if used == 4 and t.lower() != "l":
                    self.salida_consola("Limite de particiones alcanzado\n")
                    raise RuntimeError("Limite de particiones alcanzado")
                elif ext == 1 and t.lower() == "e":
                    self.salida_consola("Solo se puede crear una particion Extendida.\n")
            ...
            if ext == 0 and t.lower() == "l":
                self.salida_consola("No existe particion Extendida para crear la Logica\n")
            ...
            try:
                ...
                self.salida_consola(f"FDISK - El nombre: {n} ya existe en el disco\n")
                return
            except Exception as e:
                self.salida_consola(str(e))
                print(e)
            ...
            with open(p, "rb+") as bfile:
                ...
                if t.lower() == "e":
                    ...
                    self.salida_consola(f"FDISK - partición extendida: {n} creada correctamente\n")
                    return
                print("FDISK", "partición primaria:", n, "creada correctamente")
                self.salida_consola(f"FDISK - partición primaria: {n} creada correctamente\n")
                # print(mbr.__bytes__())
                # print(f"Temporal name: {mbr.mbr_Partition_1.part_name}")
        except ValueError as e: 
            print("FDISK", "-size debe ser un entero")
            self.salida_consola("FDISK", "-size debe ser un entero\n")
        except Exception as e: 
            self.salida_consola(f"FDISK: {str(e)} \n")
            print("FDISK", str(e))
                ...
                for ebr in ebrs:
                    if ebr.part_status == '1':
                        if ebr.part_name == name:
                            ...
                            return tmp
            self.salida_consola(f"Creando la particion {name} ... \n")
