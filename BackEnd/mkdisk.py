    ...
    def create(self):
        ...
        try:
            ...
            if os.path.exists(self.path):
                ...
                self.salida_consola(f"\tERROR: Disco ya existente en la ruta: {self.path}\n")
                return
            ...
            if not self.path.endswith(".dsk"):
                ...
                self.salida_consola(f"\tERROR: Extensión de archivo no válida para la creación del Disco.\n")
                return
            ...
            try:
                ...
                self.salida_consola(f"\t==== MKDISK: Disco creado exitosamente ====\n")
            except Exception as e:
                print(e)
                print("\tERROR: Error al crear el disco en la ruta: "+self.path)
                self.salida_consola(f"\tERROR: Error al crear el disco en la ruta: {self.path}\n")
        except ValueError:
            print("\tERROR: El parámetro size del comando MKDISK debe ser un número entero")
            self.salida_consola(f"\tERROR: El parámetro size del comando MKDISK debe ser un número entero\n")
