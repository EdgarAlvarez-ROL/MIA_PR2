...
class MKFS:
    ...
    def mkfs(self, context):
        ...
        if required:
            self.salida_consola("MKFS 2FS: requiere ciertos parámetros obligatorios\n")
    ...
    def mkfs2(self, id, t, fs):
        try:
            if t.lower() not in ["fast", "full"]:
                self.salida_consola("El comando -type necesita valores específicos.\n")
                raise ValueError("El comando -type necesita valores específicos.")
            elif fs.lower() not in ["2fs", "3fs"]:
                self.salida_consola("El comando -fs necesita valores específicos.\n")
            ...
            if fs.lower() == "2fs":
                ...
                self.salida_consola(f"MKFS: Se ha formateado la partición: {partition.part_name} con formato EXT2 con éxito\n")
                ...
                self.salida_consola(f"MKFS: Se ha formateado la partición: {partition.part_name} con formato EXT3 con éxito\n")
                print("MKFS", "Se ha formateado la partición: " + partition.part_name + " con formato EXT3 con éxito")
             
        except Exception as e:
            self.salida_consola(f"MKFS: ERROR {e} \n")
            print("MKFS", e)
